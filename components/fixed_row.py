...
def radar_graph(book_data, switch):
    ...
    df_registros["vol"] = abs(df_registros["vol"]) * df_registros["tipo"].replace(
        {"Compra": 1, "Venda": -1}
    )

    #####################################################################################################################################
    try:
        df_sectors = pd.read_csv("data/sectors.csv", index_col=0)

    except:
        df_sectors = pd.DataFrame(columns=["setor", "ativo", "participacao %"])

    df_sectors.to_csv("data/sectors.csv")
    #####################################################################################################################################

    if switch:
        df_ibov_prop = df_ibov[
            df_ibov["Código"].isin(df_registros["ativo"].unique())
        ].copy()
        df_ibov_prop["Proporcao"] = df_ibov_prop["Participação"].apply(
            lambda x: x * 100 / df_ibov_prop["Participação"].sum()
        )

        #####################################################################################################################################
        df_sectors = df_ibov_prop.drop(
            [
                "Ação",
                "Tipo",
                "Qtde. Teórica",
        ...
        ibov_setor = df_ibov_prop.groupby("Setor")["Proporcao"].sum(numeric_only=True)

        df_registros = df_registros[
            df_registros["ativo"].isin(df_ibov["Código"].unique())
        ]
        df_registros.loc[:, "Participação"] = df_registros["vol"].apply(
            lambda x: x * 100 / df_registros["vol"].sum()
        )
        ...
        try:
            df_registros["setores"] = np.concatenate(
                [
                    df_ibov_prop[df_ibov_prop["Código"] == ativo]["Setor"].values
                    for ativo in df_registros["ativo"]
                ]
            )
        except:
        ...
        fig.add_trace(
            go.Scatterpolar(
                r=ibov_setor,
                theta=ibov_setor.index,
                name="IBOV",
                fill="toself",
                hovertemplate="<b>IBOV</b>"
                + "<br><b>Participação</b>: %{r:.2f}%"
                + "<br><b>Setor</b>: %{theta}<br>",
                line=dict(color=LINHAS_PREENCHIMENTO_1),
            )
        )
        fig.add_trace(
            go.Scatterpolar(
                r=df_registros,
                theta=df_registros.index,
                name="Carteira",
                fill="toself",
                hovertemplate="<b>CARTEIRA</b>"
                + "<br><b>Participação</b>: %{r:.2f}%"
                + "<br><b>Setor</b>: %{theta}<br>",
                line=dict(color=LINHAS_PREENCHIMENTO_2),
            )
        )
    else:
        df_total_ibov = (
            df_ibov.groupby("Setor")["Participação"].sum(numeric_only=True) * 100
        )
        fig = go.Figure()
        fig.add_trace(
            go.Scatterpolar(
                r=df_total_ibov,
                theta=df_total_ibov.index,
                name="Carteira",
                fill="toself",
                hovertemplate="<b>IBOV</b>"
                + "<br><b>Participação</b>: %{r:.2f}%"
    ...
    fig.update_layout(
        MAIN_CONFIG,  # Mantém as configurações principais que você já definiu em MAIN_CONFIG
        showlegend=True,
        height=TAMANHO_RADAR,
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        autosize=True,  # Adiciona a propriedade autosize para tornar o gráfico responsivo
        margin=dict(l=20, r=20, t=20, b=20),  # Ajusta as margens
        polar=dict(
            bgcolor=BACKGROUND_RADAR,
            angularaxis=dict(
                tickfont_size=TAMANHO_INDICADORES,
                color=INDICADORES,
                gridcolor=LINHA_Y,
                linecolor=LINHA_CIRCULAR_EXTERNA,
            ),
            radialaxis=dict(
                color=AXIS_X_VALUES_COLOR,
                gridcolor=LINHAS_CIRCULARES,
                linecolor=LINHA_X,
            ),
        ),
    )
...
def update_cards_ativos(historical_data, period, dropdown, book_data):
    ...
    df2 = df_book.groupby(by=["ativo", "tipo"])["vol"].sum(numeric_only=True)
    ...
    ativos_existentes["IBOV"] = (
        1  # botei 1 pq era só pra adicionar um valor qualquer, o que importa é a chave 'IBOV'
    )
    ...
    for key, value in ativos_existentes.items():
        df_auxiliar = df_hist[df_hist.symbol.str.contains(key)].copy()
        df_auxiliar["datetime"] = pd.to_datetime(
            df_auxiliar["datetime"], format="%Y-%m-%d %H:%M:%S"
        )
    ...
    for ativo in range(len(dfativos)):
        ...
        if variacao_ativo < 0:
            lista_valores_ativos.append(
                [tag_ativo, valor_ativo, variacao_ativo, seta_caindo[0], seta_caindo[1]]
            )
        else:
            lista_valores_ativos.append(
                [
    ...
    df_hist["datetime"] = pd.to_datetime(
        df_hist["datetime"], format="%Y-%m-%d %H:%M:%S"
    )
    ...
    for n, ticker in enumerate(lista_tags):
        ...
        df_aux = df_aux.dropna()  # Removido o inplace=True
        df_aux["close"] = (
            df_aux["close"] / df_aux["close"].iloc[0] - 1
        )  # Modificado para evitar operação in-place
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name=ticker,
                line=dict(color=CARD_GRAPHS_LINE_COLOR),
                hoverinfo="skip",
            )
        )
        fig.update_layout(
                                    ...
                                    dbc.Row(
                                        [
                                            ...
                                            dbc.Col(
                                                [
                                                    dcc.Graph(
                                                        figure=lista_graficos[n],
                                                        config={
                                                            "displayModeBar": False,
                                                            "showTips": False,
                                                        },
                                                        className="graph_cards",
                                                    ),
                                                ],
                                                md=8,
                                            ),
    ...
    card_ativos = dbc.Row([*lista_colunas])

    valor_ibov = (
        html.Legend(
            ["", "{:,.2f}".format(dfativos["Value0"].iloc[-1]), " "],
            className="textoQuartenario",
        ),
    )
    ...
    if ibov_percentual < 0:
        percent_ibov = html.Legend(
            [
                html.I(className=seta_caindo[0]),
                " ",
                "{:,.2f}".format(ibov_percentual),
                "%",
            ],
            className=seta_caindo[1],
        )
    else:
        percent_ibov = html.Legend(
            [
                html.I(className=seta_crescendo[0]),
                " ",
                "{:,.2f}".format(ibov_percentual),
                "%",
            ],
            className=seta_crescendo[1],
        )
    ...
    if "Venda" in compra_e_venda.groups and "Compra" in compra_e_venda.groups:
        valor_carteira_original = (
            df_compra_e_venda["valor_total"]["Compra"]
            - df_compra_e_venda["valor_total"]["Venda"]
        )
    ...
    if valor_carteira_original == "0.00":
        ...
        varicao_carteira_configurado = (
            valor_total / float(valor_carteira_original) * 100 - 100
        )

    valor_carteira_atual = html.Legend(
        "R$" + "{:,.2f}".format(valor_carteira_original * varicao_carteira),
        className="textoSecundario",
    )

    if varicao_carteira < 1:
        percentual_carteira = html.Legend(
            [
                html.I(className=seta_caindo[0]),
                " ",
                "{:,.2f}".format(varicao_carteira_configurado),
                "%",
            ],
            className=seta_caindo[1],
        )
