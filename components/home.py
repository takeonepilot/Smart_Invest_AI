...
def line_graph(dropdown, period, profit_switch, book_info, historical_info):
    ...
    if profit_switch:
        ...
        for n, ticker in enumerate(dropdown):
            ...
            df_aux = df_aux.dropna().copy()
            df_aux.loc[:, "close"] = df_aux["close"] / df_aux["close"].iloc[0] - 1
            fig.add_trace(
                go.Scatter(
                    x=df_aux.datetime,
                    y=df_aux.close * 100,
                    mode="lines",
                    name=ticker,
                    line=dict(color=LISTA_DE_CORES_LINHAS[n]),
                )
            )
        ...
        df_ibov.loc[:, "close"] = df_ibov["close"] / df_ibov["close"].iloc[0] - 1
        fig.add_trace(
            go.Scatter(
                x=df_ibov.datetime,
                y=df_ibov.close * 100,
                mode="lines",
                name="IBOV",
                line=dict(color=LISTA_DE_CORES_LINHAS[1]),
            )
        )

        fig.add_trace(
            go.Scatter(
                x=df_patrimonio.index,
                y=(df_patrimonio["evolucao_cum"] - 1) * 100,
                mode="lines",
                name="Evolução Patrimonial",
                line=dict(color=LINHA_EVOLUCAO_PATRIMONIAL),
            )
        )
    ...
    fig.update_xaxes(
        tickfont=dict(family="Nexa", size=AXIS_FONT_SIZE, color=AXIS_VALUES_COLOR),
        gridcolor=LINHAS_DE_GRADE,
    )
    fig.update_yaxes(
        tickfont=dict(family="Nexa", size=AXIS_FONT_SIZE, color=AXIS_VALUES_COLOR),
        gridcolor=LINHAS_DE_GRADE,
        zerolinecolor=LINHA_ZERO_X,
    )
