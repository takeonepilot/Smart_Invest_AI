...
card_sem_registros = dbc.Card(
    [
        dbc.CardBody(
            [
                html.Legend(
                    "Nenhum registro efetuado", className="textoQuartenarioBranco"
                )
            ]
        )
    ],
                                            ...
                                            dbc.Col(
                                                [
                                                    dbc.Button(
                                                        [
                                                            html.I(
                                                                className="fa fa-trash header-icon",
                                                                style={
                                                                    "font-size": "150%"
                                                                },
                                                            )
...
@app.callback(Output("layout_wallet", "children"), Input("layout_data", "data"))
    ...
    # 2. Salvando ativo
        ...
        if None in [ativo, preco, vol] and open:
            ...
            if tv.search_symbol(ativo, "BMFBOVESPA"):
                exchange = "BMFBOVESPA"
                preco = round(preco, 2)
                df_book_data.loc[len(df_book_data)] = [
                    periodo,
                    preco,
                    radio,
                    ativo,
                ...
                df_book_data["date"] = pd.to_datetime(
                    df_book_data["date"], format="%Y-%m-%d"
                )
                df_book_data.reset_index(drop=True, inplace=True)
                df_book_data = df_book_data.sort_values(by="date", ascending=True)

                df_book_data.to_csv("data/book_data.csv")
