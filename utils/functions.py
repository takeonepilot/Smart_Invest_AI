...
df_ibov = pd.read_csv("data/tabela_ibov.csv")

# trata os valores de cada coluna do df_ibov
df_ibov["Part. (%)"] = pd.to_numeric(df_ibov["Part. (%)"].str.replace(",", "."))
df_ibov["Qtde. Teórica"] = pd.to_numeric(
    df_ibov["Qtde. Teórica"].str.replace(".", "", regex=False)
)
...
df_ibov["Setor"] = df_ibov["Setor"].apply(
    lambda x: "Cons N Cíclico" if x == "Cons N Ciclico" else x
)


def definir_evolucao_patrimonial(
    df_historical_data: pd.DataFrame, df_book_data: pd.DataFrame
) -> pd.DataFrame:
    ...
    df_historical_data = (
        df_historical_data.groupby(["date", "symbol"])["close"]
        .last()
        .to_frame()
        .reset_index()
    )
    ...
    df_historical_data = df_historical_data.pivot(
        values="close", columns="symbol", index="date"
    )
    ...
    df_book_data["vol"] = df_book_data["vol"] * df_book_data["tipo"].replace(
        {"Compra": 1, "Venda": -1}
    )
    ...
    df_patrimonio["soma"] = df_patrimonio.sum(axis=1, numeric_only=True)
    ...
    df_patrimonio["evolucao_patrimonial"] = df_patrimonio["soma"].diff() - df_ops.sum(
    ...
    df_patrimonio["evolucao_percentual"] = (
        df_patrimonio["evolucao_patrimonial"] / df_patrimonio["soma"]
    )
    ...
    df_patrimonio["evolucao_percentual"] = df_patrimonio["evolucao_percentual"].fillna(
        0
    )
...
try:
    df_book_data = pd.read_csv("data/book_data.csv", index_col=0)
...
def iterar_sobre_df_book(df_book_var: pd.DataFrame, ativos_org_var={}) -> dict:
    for _, row in df_book_var.iterrows():
        if row["ativo"] not in ativos_org_var:
            ativos_org_var[row["ativo"]] = {"exchange": row["exchange"]}

    ativos_org_var["IBOV"] = {"exchange": "BMFBOVESPA"}
    return ativos_org_var


def atualizar_historical_data(
    df_historical_var: pd.DataFrame, ativos_org_var={}
) -> pd.DataFrame:
    tv = TvDatafeed(username_td, password_td)
    for symb, params in ativos_org_var.items():
        try:
            # Assumindo que symb_dict é um dicionário com o símbolo como chave e os parâmetros como valor
            new_line = tv.get_hist(symb, **params, n_bars=5000)
            if new_line is not None and not new_line.empty:
                # Apenas selecionando as colunas 'symbol' e 'close' se new_line não for None e não estiver vazio
                new_line = new_line[["symbol", "close"]].reset_index()
                df_historical_var = pd.concat(
                    [df_historical_var, new_line], ignore_index=True
                )
            else:
                # Pode logar uma mensagem ou fazer outra ação caso new_line seja None ou vazio
                print(f"Nenhum dado retornado para {symb} com parâmetros {params}")
        except Exception as e:
            # Tratamento de possíveis erros durante a chamada de get_hist
            print(
                f"Erro ao obter dados históricos para {symb} com parâmetros {params}: {e}"
            )
